...
class Story:
    def __init__(self):
        self.__api_key = "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" # This is the key for the openAI API
        self.client = OpenAI(api_key=self.__api_key) # This is the client for the openAI API
        self.keysToTry = 1 # This is the index of the key to try, init to 1 because we're using 0
        with open("api_keys.txt", "r") as self.keys:
            self.keys = self.keys.readlines()
            

    def fix_key(self):
        self.__api_key = self.keys[self.keysToTry].strip()
        self.client = OpenAI(api_key=self.__api_key)
        self.keysToTry = self.keysToTry + 1
    ...
    def make_story(self):
        ...
        try: # This is to catch any errors in the JSON
